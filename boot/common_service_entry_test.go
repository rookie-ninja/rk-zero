...
func TestCommonServiceEntry_Entries_HappyCase(t *testing.T) {
	...
	zeroEntry := RegisterZeroEntry(
		WithCommonServiceEntryZero(entry),
		WithNameZero("unit-test-zero"))
	rkentry.GlobalAppCtx.AddEntry(zeroEntry)
	defer rkentry.GlobalAppCtx.RemoveEntry(zeroEntry.EntryName)
