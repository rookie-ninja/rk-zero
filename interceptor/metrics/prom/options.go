...
func getValues(req *http.Request, writer http.ResponseWriter) []string {
	...
	if req != nil {
		...
		if writer != nil {
			if v, ok := writer.(*rkzerointer.RkResponseWriter); ok {
	...
	values := []string{
		entryName,
		entryType,
		rkzerointer.Realm.String,
		rkzerointer.Region.String,
		rkzerointer.AZ.String,
		rkzerointer.Domain.String,
		rkzerointer.LocalHostname.String,
...
func getOptionSet(req *http.Request) *optionSet {
	...
	if v := req.Context().Value(rkzerointer.RpcEntryNameKey); v != nil {
		entryName = v.(string)
	}
