...
func (set *optionSet) Tick(req *http.Request, writer http.ResponseWriter, next http.HandlerFunc) error {
	var err error
	...
	bufPool := &bufferPool{}
	buffer := bufPool.Get()
	originalWriter := writer.(*rkzerointer.RkResponseWriter).Writer
	newWriter := newWriter(originalWriter, buffer)

	// 3: assign new writer
	// writer must be RkResponseWriter
	writer.(*rkzerointer.RkResponseWriter).Writer = newWriter
	...
	case recv := <-panicChan:
		...
		writer.(*rkzerointer.RkResponseWriter).Writer = originalWriter
		panic(recv)
